    ...
    <a
      ...
      <div className={`w-full grow overflow-hidden space-y-0.5 block`}>
        ...
        <div className="w-full min-h-[30px] pt-1 flex flex-wrap space-x-1.5">
          ...
          {bookmark.folder_bookmarks &&
            bookmark.folder_bookmarks.length > 0 &&
            bookmark.folder_bookmarks.map((folder) => (
              <Badge
                key={folder.folders.id}
                variant={'outline'}
                className={cn(`text-[11px] flex-none btn-action`)}>
                <Avatar className="w-4 h-4 aspect-square cursor-pointer mr-1">
                  <AvatarImage
                    src={folder.folders.icon || ''}
                    alt={folder.folders.name || ''}
                  />
                  <AvatarFallback className="invert">
                    {folder.folders.name ? folder.folders.name[0] : ''}
                  </AvatarFallback>
                </Avatar>
                <span>{folder.folders.name}</span>
              </Badge>
            ))}
